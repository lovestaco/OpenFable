...
**Step 5 (freeze `e`)** is the key architectural invariant: the encoded input `e` is captured after the Prelude and injected at _every_ loop iteration unchanged. This prevents the hidden state from drifting away from the original input signal regardless of loop depth.
